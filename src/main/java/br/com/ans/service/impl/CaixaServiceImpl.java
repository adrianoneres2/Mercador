		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(
					...
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro!",
							"Erro ao tentar recuperar o caixa!"));
		}
	...
	public Caixa fecharCaixa(Caixa caixa) {
		...
		if (caixaAberto == null) {
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Info",
							"Não existe caixa aberto para este operador!"));
			return null;
		} else {
			try {
				caixaAberto.setUsuarioFechamento(caixa.getUsuarioFechamento());
				 if(vendaServiceImpl.buscarVenda(caixa.getUsuarioOperador()).getCodigoVenda() != null){
						FacesContext.getCurrentInstance().addMessage(
								null,
								new FacesMessage(FacesMessage.SEVERITY_INFO, "Alerta!",
										"Existe venda aberta pera este operador!!"));
